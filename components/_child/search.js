    ...
    <form onSubmit={handleSubmit}>
        <div className="flex items-center">
            <div className="relative"> 
                ...
                <button className="absolute top-0 right-5 bottom-0 m-auto" disabled={!searchInput.trim()} type="submit"><FaSearch className={`${isSearchDisabled ? "text-slate-400 dark:text-slate-400" : "hover:cursor-pointer hover:opacity-70"} dark:text-white`} /></button>  
            </div>
        </div>
    </form>
  );
